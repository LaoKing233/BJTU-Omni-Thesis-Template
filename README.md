...
> 🏆 模板作者已通过本模板撰写的论文（实践成果总结报告）成功毕业，并获评校级优秀学位论文（实践成果）。
...
- **2026-06-25** ✨ v1.0.1 新增参考文献导出脚本，便于向图书馆系统填写最终版论文的「参考文献」信息
- **2026-06-23** 🎉 v1.0.0 正式版发布；模板作者已通过本模板撰写的论文（实践成果总结报告）成功毕业，并获评校级优秀学位论文（实践成果）
- **2026-05-11** 🔧 v0.1.5 修复 PowerShell 字数统计脚本在 Windows 中文系统下的编码报错（添加 UTF-8 BOM）
- **2026-05-10** 🎉 以本模板撰写的论文已通过北京交通大学计算机学院论文格式审查及论文一审
- **2026-04-17** ✨ v0.1.4 数学公式字体改为 Times New Roman 风格；参考文献序号右对齐、间距优化
- **2026-04-12** 🔧 v0.1.3 修复题注中英文行距、参考文献作者姓名不再强制全大写
- **2026-04-07** ✨ v0.1.2 新增 `BlindReview` 盲审模式（隐藏校名、个人信息，整页隐藏致谢等章节）、`NumberByChapter` 编号控制选项；统一各学位类型内封信息页布局；修复致谢误入目录等问题
...
向图书馆系统上传最终版论文时，需在表单中填写「参考文献」一项。本脚本依赖 `pdftotext`（TeX Live / poppler 自带），从编译好的 `main.pdf` 中提取参考文献，每条合并为一行的纯文本，直接复制粘贴即可填写，省去手动整理。需先编译生成 `main.pdf`，再在项目根目录下运行：

**Linux/macOS：**
```bash
bash scripts/ref_export.sh
```

**Windows PowerShell：**
```powershell
scripts\ref_export.ps1
```

结果保存至 `outputs/references.txt`，每行一条参考文献，可直接复制。脚本会自动去除 URL 跨行折断处的多余空格、还原西文断字连字符。

> 📌 提取自 PDF，依赖编号与版面解析；建议上传前对照 PDF 自行复核一遍，确保无遗漏或错位。
